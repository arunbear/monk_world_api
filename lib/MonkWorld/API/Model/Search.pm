...
sub search ($self, $query, $limit = 50, $after = undef) {
    ...
    # Get the maximum node ID if after is not provided
    if (!defined $after) {
        $after = $self->pg->db->query('SELECT COALESCE(MAX(id), 0)::bigint as max_id FROM node')
                      ->hash
                      ->{max_id} + 1;
    }

    # Use PostgreSQL full-text search with websearch_to_tsquery
    my $results = $self->pg->db->query(<<~"SQL", $after, $query, $query, $limit)->hashes->to_array;
        SELECT
            n.id,
            n.title,
            n.created_at,
